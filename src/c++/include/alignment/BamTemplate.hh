...
public:
    BamTemplate();
    BamTemplate(const FragmentMetadata &read1, const FragmentMetadata &read2, const bool properPair = false);
    ...
    double getLogProbability() const
    {
        return std::accumulate(fragmentMetadataList_.begin(), fragmentMetadataList_.end(), 0,
                               boost::bind(std::plus<double>(), _1,
                                           boost::bind(&FragmentMetadata::logProbability, _2)));
    }

    unsigned getSmithWatermanScore() const
    {
        return std::accumulate(fragmentMetadataList_.begin(), fragmentMetadataList_.end(), 0,
                               boost::bind(std::plus<unsigned>(), _1,
                                           boost::bind(&FragmentMetadata::smithWatermanScore, _2)));
    }
    ...
    bool isBetterThan(const unsigned minScore, const double rog, const BamTemplate &alt) const {

        const unsigned ourScoreGivenAlt = computeAlignmentScore(rog, exp(getLogProbability()), exp(alt.getLogProbability()));
        ISAAC_THREAD_CERR_DEV_TRACE_CLUSTER_ID(getCluster().getId(),"BamTemplate::isBetterThan ourScoreGivenAlt: " << ourScoreGivenAlt << " minScore:" << minScore);
        return minScore <= ourScoreGivenAlt;
    }
