...
public:
    ...
    enum OpCode {
        ...
        SOFT_CLIP = 4, // 'S'
        HARD_CLIP = 5, // 'H' // only used to indicate internal clipping in inversions.
        PAD = 6, // 'P'
        MATCH = 7, // '='
        MISMATCH = 8, // 'X'
    ...
    void fromString(IteratorT begin, IteratorT end)
    {
        ...
        for (IteratorT i = begin; end != i; ++i)
        {
            ...
            {
                const OpCode oc = OpCode(
                    std::distance(opCodes, std::find(opCodes, &opCodes[0] + sizeof(opCodes) - 1, c)));
                addOperation(len, oc);
                len = 0;
            }
        }
     ...
     */
    template <bool hardClipsAsS = false, typename IteratorT, typename ContainerT>
    static const ContainerT &toString(IteratorT begin, IteratorT end, ContainerT &result)
    {
        for (IteratorT v = begin; end != v; ++v)
        {
            ...
            result.push_back(opCodeToChar<hardClipsAsS>(d.second));
        }
        return result;
    }
...
//        return std::pair<unsigned, OpCode>(length, static_cast<OpCode>(code));
    ...
    static unsigned getMaxLength(const unsigned maxGaps)
    {
        return getMaxOpeations(maxGaps) * sizeof(value_type);
    }
    ...
    static unsigned getMaxOpeations(const unsigned maxGaps)
    {
        ...
        const unsigned maxCigarOperations =
            maxSoftClipOps + maxHardClipOps + oneMatchOp + std::max(cigarOpsPerStructuralVariant, maxGaps * cigarOpsPerGap);

        return maxCigarOperations;
    }

    static unsigned getTotalOperationsForReads(const flowcell::ReadMetadataList &readMetadataList, const unsigned maxGaps)
    {
        return readMetadataList.size() * getMaxOpeations(maxGaps);
    }

    static unsigned getMaxOperationsForReads(
        const flowcell::FlowcellLayoutList &flowcellLayoutList,
        const unsigned maxGaps)
    {
        unsigned ret = 0;
        BOOST_FOREACH(const flowcell::Layout &flowcell, flowcellLayoutList)
        {
            ret = std::max(ret, getTotalOperationsForReads(flowcell.getReadMetadataList(), maxGaps));
        }
        return ret;
    }

    /**
     * @return encoded SOFT_CLIP if value was a hard clip, original value otherwise
     */
    static uint32_t softenHardClip(uint32_t value)
    {
        alignment::Cigar::Component c = alignment::Cigar::decode(value);
        return HARD_CLIP == c.second ?
            encode(c.first, SOFT_CLIP) : value;
    }
private:
    static uint32_t encode(const uint32_t &length, OpCode opCode)
    {
...
struct CigarPosition
{
    ...
    CigarPosition & operator ++()
    {
        ...
        else if (Cigar::HARD_CLIP == component.second)
        {
            sequenceOffset_ += component.first;
        }
        else
...
private:
    void skipToNextAlign()
    {
        while (!end())
        {
            ...
            {
                ISAAC_ASSERT_MSG(
                    Cigar::ALIGN == nextComponent.second || Cigar::SOFT_CLIP == nextComponent.second || Cigar::HARD_CLIP == nextComponent.second,
