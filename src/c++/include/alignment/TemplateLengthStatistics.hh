...
public:
    ...
    {
        if (!f1.isSplit() && !f2.isSplit() && f1.getContigId() == f2.getContigId() &&
            f1.getFStrandReferencePosition().getContigId() == f1.getRStrandReferencePosition().getContigId() &&
            f2.getFStrandReferencePosition().getContigId() == f2.getRStrandReferencePosition().getContigId())
        {
...
public:
    ...
    static constexpr double TEMPLATE_LENGTH_HIGH_CONFIDENCE_RATIO = 4.0;
private:
    ...
    /// Check if the fragment with the given orientation and readIndex is supposed to come first
...
{
public:
    TemplateLengthDistribution(const unsigned updateFrequency, int mateDriftRange);

    void clear();

    // Constructor for unit tests
    TemplateLengthDistribution(const unsigned updateFrequency,
