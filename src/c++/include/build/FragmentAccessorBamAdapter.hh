...
class FragmentAccessorBamAdapter
{
    ...
    common::StaticVector<unsigned, 1024> currentCigar_;
    bool reverse_;
    ...
    /**
     * true if the cigar changed from the original. Currently there are two situations where this can happen:
...
public:
        ...
        const io::FragmentAccessor& fragment)
    {
        ...
        currentCigar_.clear();
        std::transform(
            index.cigarBegin_, index.cigarEnd_, std::back_inserter(currentCigar_),
            alignment::Cigar::softenHardClip);
        ...
        primaryAlignment_ = !index.splitInfoCount_ || makeSaTagString(
            pos_,
            reverse_,
            index.cigarBegin_, index.cigarEnd_,
            mapq(),
        ...
        cigarChanged_ = index.cigarBegin_ != pFragment_->cigarBegin();
        ...
        const io::FragmentAccessor& fragment)
    {
        ...
        saTagBuffer_.clear();
        saNM_ = -1U;
        // For unaligned reads this prevents from bogus empty OC:Z being included with unaligned data
        cigarChanged_ = false;
        return *this;
    }
    ...
    bam::zTag getFragmentOC() {
        ...
        if (!includeTags_.includeOC_ || !cigarChanged_)
        {
            return bam::zTag();
        }
    ...
    bam::iTag getFragmentOP() {
        static const char OP[2] = {'O', 'P'};

        if (!includeTags_.includeOC_ || pFragment_->fStrandPosition_ == pFragment_->fStrandOriginalPosition_)
        {
            return bam::iTag();
        }
        // tag position is 1-based
        return bam::iTag(OP, pFragment_->getOriginalPosition() + 1);
    }
    ...
    CigarBeginEnd cigar() const {
        return std::make_pair(currentCigar_.begin(), currentCigar_.end());
    }
    ...
    {
        if (pFragment_->flags_.unmapped_)
        {
            // picard 1.140 complains if unmapped reads have non-zero mapq
            return 0;
        }
        else if (alignment::UNKNOWN_MAPQ == pFragment_->mapQ_)
        {
            return forcedDodgyAlignmentScore_;
        }
        return pFragment_->mapQ_;
    }
    ...
    bam::iTag getFragmentAS() const {
        ...
        return (!includeTags_.includeAS_) ?
    ...
    int observedLength() const { return alignment::computeObservedLength(currentCigar_.begin(), currentCigar_.end()); }
};
