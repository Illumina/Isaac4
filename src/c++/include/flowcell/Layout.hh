...
inline size_t getMaxSeedsPerRead(const flowcell::Layout& flowcell, const unsigned seedLength)
{
    size_t maxNonOverlappingSeeds = 0;
    for (const flowcell::ReadMetadata& read : flowcell.getReadMetadataList())
    {
        maxNonOverlappingSeeds = std::max<std::size_t>(maxNonOverlappingSeeds, read.getLength() / seedLength);
    }
    return maxNonOverlappingSeeds;
}

inline size_t getMaxSeedsPerRead(
    const flowcell::FlowcellLayoutList &flowcellLayoutList,
    const unsigned seedLength)
{
    size_t maxNonOverlappingSeeds = 0;
    for (const flowcell::Layout &flowcell : flowcellLayoutList)
    {
        maxNonOverlappingSeeds = std::max<std::size_t>(maxNonOverlappingSeeds, getMaxSeedsPerRead(flowcell, seedLength));
    }
