...
private:
    ...
    // allocations.
    common::PathStringType filePath_;
...
public:
    ...
    void reopen(const common::PathCharType* filePath, FileBufWithReopen::FadviseFlags fadvise)
    {
        if (!fileBufPtr_->reopen(filePath, fadvise)) {
            BOOST_THROW_EXCEPTION(common::IoException(errno, std::string("Failed to reopen a file handle for ") + common::pathStringToStdString(filePath)));
        }
    ...
    const common::PathStringType& path() const {return filePath_;}
