...
enum Mode
{
    ...
    ForwardNoPermutate, // Produce forward kmers, don't permutate.
...
//    ISAAC_THREAD_CERR << "endGenomicOffset:" << endGenomicOffset << std::endl;
    ...
    BOOST_FOREACH(const ContigList::Contig &contig, contigList_)
    {
        ...
        if (contig.size() > beginGenomicOffset)
        {
//            ISAAC_THREAD_CERR << "Generating kmers for " << contig << std::endl;

            const std::size_t threadSectionLengthWithOverlap = threadSectionLength + oligo::KmerTraits<KmerT>::KMER_BASES - 1;
            ...
            oligo::KmerGenerator<oligo::KmerTraits<KmerT>::KMER_BASES, KmerT, ContigList::Contig::const_iterator> kmerGenerator(
    ...
    //            ISAAC_THREAD_CERR << "  genomicOffset:" << "  generatorOffset:" << generatorOffset << "  generatorEnd:" << generatorEnd << std::endl;
            ...
            KmerT kmer(0);
            for (ContigList::Contig::const_iterator it = kmerGenerator.next(kmer);
...
//                {
//                    ISAAC_THREAD_CERR << " Generated kmer:" << oligo::Bases<oligo::BITS_PER_BASE, KmerT>(kmer, oligo::KmerTraits<KmerT>::KMER_BASES) <<
//                        " at position:" << kmerPosition << std::endl;
//                }
//                generate(kmer, permutate, boost::bind(callback, threadNumber, _1, contig.getIndex(), kmerPosition, _2));
                generate(
                    kmer, permutate,
                    [callback, threadNumber, &contig, kmerPosition](const KmerT kmer, const bool reverse)
                    {
                        callback(threadNumber, kmer, contig.getIndex(), kmerPosition, reverse);
                    });
                ++generated;
            }
        }
