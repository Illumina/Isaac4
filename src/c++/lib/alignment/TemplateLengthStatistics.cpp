...
bool TemplateLengthStatistics::mateOrientation(unsigned readIndex, bool reverse) const
{
    ...
    // for discordant models, return the expected orientation in the best model
    ...
    const unsigned *readLengths) const
{
    ...
    {
        return position + mateMin_ / TEMPLATE_LENGTH_HIGH_CONFIDENCE_RATIO - readLengths[(readIndex + 1) % 2];
    }
    else
    {
        return position - mateMax_ * TEMPLATE_LENGTH_HIGH_CONFIDENCE_RATIO + readLengths[readIndex];
    }
}
    ...
    const unsigned *readLengths) const
{
    ...
    if (firstFragment(reverse, readIndex))
    {
        return position + mateMax_ * TEMPLATE_LENGTH_HIGH_CONFIDENCE_RATIO - readLengths[(readIndex + 1) % 2];
    }
    else
    {
        return position - mateMin_ / TEMPLATE_LENGTH_HIGH_CONFIDENCE_RATIO + readLengths[readIndex];
    }
}



TemplateLengthDistribution::TemplateLengthDistribution(const unsigned updateFrequency, int mateDriftRange)
    ...
    const uint64_t length)
{
    ...
    {
        ++count_;
//        ISAAC_THREAD_CERR << "TLS " << length  << " " << count_ << std::endl;
    }
    // calculate the alignment statistics if appropriate
    if (0 == (count_ % updateFrequency_))
    {
        updateStatistics();
    }
}
