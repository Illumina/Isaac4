...
void TestTemplateLengthStatistics::testAlignmentModels()
{
    ...
    f1.contigId = 0;
    f2.contigId = 0;
    f1.rStrandPos = isaac::reference::ReferencePosition(f1.contigId, f1.position + 1);
    f2.rStrandPos = isaac::reference::ReferencePosition(f2.contigId, f2.position + 1);
...
void TestTemplateLengthStatistics::addTemplates(isaac::alignment::TemplateLengthDistribution & tls)
{
    ...
    f[0][0].contigId = 0;
    f[0][0].position = 0;
    f[0][0].rStrandPos = isaac::reference::ReferencePosition(f[0][0].contigId, f[0][0].position + 1);
    f[0][0].reverse = false;
    ...
    f[1][0].rStrandPos = f[0][0].rStrandPos;
    for (unsigned int i = 1; i < 10000; ++i)
    {
        CPPUNIT_ASSERT_EQUAL(false, tls.appendTemplate(f[0], f[1]));
        ++f[1][0].position;
...
void TestTemplateLengthStatistics::testStatistics()
{
    ...
    using isaac::alignment::FragmentMetadata;
    TemplateLengthDistribution tls(10000, -1);
    addTemplates(tls);
    CPPUNIT_ASSERT_EQUAL(14U, tls.getStatistics().getMin());
    CPPUNIT_ASSERT_EQUAL(5001U, tls.getStatistics().getMedian());
    CPPUNIT_ASSERT_EQUAL(9987U, tls.getStatistics().getMax());
...
void TestTemplateLengthStatistics::testMateDriftRange()
{
    using isaac::alignment::TemplateLengthDistribution;
    using isaac::alignment::FragmentMetadata;
    TemplateLengthDistribution tls(10000, 123);
    addTemplates(tls);
    CPPUNIT_ASSERT_EQUAL(5001U, tls.getStatistics().getMedian());
...
void TestTemplateLengthStatistics::testMateMinPosition()
{
    ...
    using isaac::alignment::FragmentMetadata;
    {
        ...
        CPPUNIT_ASSERT_EQUAL(442L, tls.mateMinPosition(0, false, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(-233L, tls.mateMinPosition(0, true, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(458L, tls.mateMinPosition(1, false, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(-217L, tls.mateMinPosition(1, true, 500, readLengths));
    }
    {
        TemplateLengthStatistics tls(100, 200, 170, 160, 175, TemplateLengthStatistics::RFp, TemplateLengthStatistics::FRm, -1);
        const unsigned readLengths[] = {67, 83};
        CPPUNIT_ASSERT_EQUAL(-233L, tls.mateMinPosition(0, false, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(442L, tls.mateMinPosition(0, true, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(-217L, tls.mateMinPosition(1, false, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(458L, tls.mateMinPosition(1, true, 500, readLengths));
    }
    {
        TemplateLengthStatistics tls(100, 200, 170, 160, 175, TemplateLengthStatistics::FRm, TemplateLengthStatistics::RFp, -1);
        const unsigned readLengths[] = {67, 83};
...
void TestTemplateLengthStatistics::testMateMaxPosition()
{
    ...
    using isaac::alignment::FragmentMetadata;
    {
        TemplateLengthStatistics tls(100, 200, 170, 160, 175, TemplateLengthStatistics::FRp, TemplateLengthStatistics::RFm, -1);
        const unsigned readLengths[] = {67, 83};
        CPPUNIT_ASSERT_EQUAL(1217L, tls.mateMaxPosition(0, false, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(542L, tls.mateMaxPosition(0, true, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(1233L, tls.mateMaxPosition(1, false, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(558L, tls.mateMaxPosition(1, true, 500, readLengths));
    }
    {
        TemplateLengthStatistics tls(100, 200, 170, 160, 175, TemplateLengthStatistics::RFp, TemplateLengthStatistics::FRm, -1);
        const unsigned readLengths[] = {67, 83};
        CPPUNIT_ASSERT_EQUAL(542L, tls.mateMaxPosition(0, false, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(1217L, tls.mateMaxPosition(0, true, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(558L, tls.mateMaxPosition(1, false, 500, readLengths));
        CPPUNIT_ASSERT_EQUAL(1233L, tls.mateMaxPosition(1, true, 500, readLengths));
    }
    {
        TemplateLengthStatistics tls(100, 200, 170, 160, 175, TemplateLengthStatistics::FRm, TemplateLengthStatistics::RFp, -1);
        const unsigned readLengths[] = {67, 83};
