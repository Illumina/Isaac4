    ...
    FragmentMetadata &fragmentMetadata)
{
    ...
    if (Cigar::ALIGN == operation.second)
    {
        ...
        const reference::Contig &reference = contigList.at(fragmentMetadata.contigId);
        std::reverse_iterator<reference::Contig::const_iterator> referenceRBegin(reference.begin() + fragmentMetadata.rStrandPos.getPosition());
        ...
        {
            fragmentMetadata.cigarOffset = cigarBuffer_.size();
            fragmentMetadata.rStrandPos -= clipped.first;
