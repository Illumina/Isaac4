    ...
    PackedFragmentBuffer &dataBuffer)
{
    ...
    if (!index.hasMate() || fragment.flags_.mateUnmapped_)
    {
        ...
        if (index.hasMate())
        {
            ...
            ISAAC_VERIFY_MSG(mate.bamTlen_ == -oldBamTlen,
                             "Incoherent BAM template length between fragment and unmapped mate\n" << index <<
                             "\n" << fragment << " mate:\n" << mate);
    ...
    // these fields are shared information and updated under the exclusive lock, make sure it is in sync or else something bad has happened
    ISAAC_ASSERT_MSG(mate.bamTlen_ == -fragment.bamTlen_,
                     "Incoherent BAM template length between fragment and mate\n" << index <<
                     "\n" << fragment <<
                     " mate:\n" << mate);

    ISAAC_ASSERT_MSG(mate.mateFStrandPosition_ == fragment.fStrandPosition_,
                     "Incoherent mateFStrandPosition_ between fragment and mate\n" << index <<
                     "\n" << fragment <<
                     " mate:\n" << mate);

    ISAAC_ASSERT_MSG(mate.fStrandPosition_ == fragment.mateFStrandPosition_,
                     "Incoherent fStrandPosition_ between fragment and mate\n" << index <<
                     "\n" << fragment <<
                     " mate:\n" << mate);
...
bool GapRealigner::verifyGapsChoice(
    const GapChoiceBitmask &choice,
    const gapRealigner::GapsRange& gaps,
    const reference::ReferencePosition& binStartPos,
    const reference::ReferencePosition& binEndPos,
...
bool GapRealigner::findBetterGapsChoice(
    const gapRealigner::GapsRange& gaps,
    const reference::ReferencePosition& binStartPos,
    const reference::ReferencePosition& binEndPos,
    const reference::ContigList& reference,
...
//            const gapRealigner::OverlappingGapsFilter overlappingGapsFilter(gaps);
    ...
    for (GapChoiceBitmask choice = 0; (choice = gapsFilter.next(choice));)
    {
        ...
        ret |= verifyGapsChoice(choice, gaps, binStartPos, binEndPos, fragment, reference, originalMismatchesPercent, undoneAlignmentPos, bestChoice);
    ...
    const reference::ContigLists &contigLists)
{
    ...
    if (!realignDodgyFragments_ && alignment::UNKNOWN_MAPQ == fragment.mapQ_)
    {
    ...
    do
    {
        ...
        const gapRealigner::GapsRange gaps = realignerGaps.findGaps(fragment.clusterId_, binStartPos, bounds.beginPos_, bounds.endPos_, 8, currentAttemptGaps_);
...
//        }
        ...
        // false means none of the gaps apply. It also means the original alignment should be kept.
        if (findBetterGapsChoice(gaps, binStartPos, binEndPos, reference, fragment, index, leftToEvaluate, bestChoice))
        {
