...
inline bool orderByGapPriority(const gapRealigner::Gap &left, const gapRealigner::Gap &right)
{
...
void RealignerGaps::finalizeGaps()
{
    ...
    // keep unique gaps only. increment priority to reflect the number of reads supportin each gap
    gapRealigner::Gaps::iterator uniq = gapGroups_.begin();
    for (gapRealigner::Gaps::iterator current = gapGroups_.begin() + 1;
        gapGroups_.end() != current; ++current)
    {
        if (gapRealigner::Gap::comparePositionAndLength(*uniq, *current))
        {
            // at the moment capacity is 2^32, so no need to check for overrun
            ++uniq->priority_;
        }
        else
        {
            ++uniq;
            using std::swap; swap(*uniq, *current);
        }
    }
    ++uniq;
    gapGroups_.erase(uniq, gapGroups_.end());
...
//    ISAAC_THREAD_CERR_DEV_TRACE("findGaps all gaps: " << gapRealigner::GapsRange(sampleGaps.begin(), sampleGaps.end()));
    ...
    else
    {
        ...
        if (maxGaps < foundGaps.size())
        {
            std::sort(foundGaps.begin(), foundGaps.end(), orderByGapPriority);
            foundGaps.erase(foundGaps.begin() + maxGaps, foundGaps.end());
        }
    }
