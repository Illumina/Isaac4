    ...
    boost::filesystem::path &result)
{
    ...
    if (patternedFlowcell)
    {
        ...
        result /= boost::filesystem::path("..");
        result /= boost::filesystem::path(forceClocs ? "s.clocs" : "s.locs");
    }
    else
    {
    ...
    //    std::string & pathInternalStringRef = const_cast<std::string&>(result.string());
        ...
        result = baseCallsPath.c_str();
        result /= boost::filesystem::path(laneFolder);
        result /= boost::filesystem::path(filterFileName);
...
//    pathInternalStringRef.append(bclFileName, bclFileName + bclFileNameLength);
    result = baseCallsPath.c_str();
    result /= boost::filesystem::path(laneFolder);
    result /= boost::filesystem::path(cycleFolder);
    result /= boost::filesystem::path(bclFileName);
