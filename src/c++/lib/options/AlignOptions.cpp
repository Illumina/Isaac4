...
AlignOptions::AlignOptions()
    ...
    , enableNuma(false)
    , candidateMatchesMax(800)
    , matchFinderTooManyRepeats(4000)
    ...
    , smitWatermanGapsMax(realignedGapsPerFragment)
    , useSmithWaterman("smart")
    , smartSmithWaterman(true)
    , smithWatermanGapSizeMax(16)
    ...
    , tempLoadersMax(4) // assuming the temporary data sits on a low-latency storage (local spinning disk or ssd, reduce the competition for reading to increase the throughput.
    ...
    , anchorMate(true)
    , binRegexString("all")
    , decoyRegexString("decoy")  //("(?!.*)") // negative lookahead regex should not match anything. So nothing is a decoy by default
    ...
    , detectTemplateBlockSize(10000)
    , disableResume(false)
{
    static bool bufferBins = false;
    ...
    namedOptions_.add_options()
        ...
        ("realign-mapq-min"     , bpo::value<unsigned>(&realignMapqMin)->default_value(realignMapqMin),
                "Gaps from alignments with lower MAPQ will not be used as candidates for gap realignment")
        ("known-indels"           , bpo::value<std::string>(&knownIndelsPathString),
        ...
        ("detect-template-block-size" , bpo::value<unsigned>(&detectTemplateBlockSize)->default_value(detectTemplateBlockSize),
            "Number of pairs to use as a single block for template length statistics detection")
        ...
        ("seed-length"              , bpo::value<unsigned>(&seedLength)->default_value(seedLength),
            ("Length of the seed in bases. Only " + oligo::supportedKmersString() +
            " are allowed. Longer seeds reduce sensitivity on noisy data but improve repeat resolution and run time.").c_str())
        ...
        ("target-bin-size"            , bpo::value<uint64_t>(&targetBinSizeMB)->default_value(targetBinSizeMB),
        ...
        ("reference-genome,r"       , bpo::value<std::string>(&sortedReferenceXmlString),
                "Full path to the reference genome XML descriptor."
            )
        ("reference-name,n"       , bpo::value<std::string>(&referenceName)->default_value(referenceName),
        ...
        ("candidate-matches-max"                   , bpo::value<std::size_t>(&candidateMatchesMax)->default_value(candidateMatchesMax),
                "Maximum number of candidate matches to be considered for finding the best alignment. If seeds yield a greater number, "
                "the alignment generally is not performed. Other mechanisms such as shadow rescue may still place the fragment.")
        ("match-finder-too-many-repeats"                   , bpo::value<unsigned>(&matchFinderTooManyRepeats)->default_value(matchFinderTooManyRepeats),
                "Maximum number of seed matches to be looked at for each attempted seed")
        ("match-finder-way-too-many-repeats"                   , bpo::value<unsigned>(&matchFinderWayTooManyRepeats)->default_value(matchFinderWayTooManyRepeats),
                "Maximum number of seed matches to be looked at if in a pair one read has candidate alignments and the other"
                "has gone over match-finder-too-many-repeats on all seeds or over candidate-matches-max when seed position merge was attempted ")
        ("match-finder-shadow-split-repeats"                   , bpo::value<unsigned>(&matchFinderShadowSplitRepeats)->default_value(matchFinderShadowSplitRepeats),
                "Maximum number of seed candidate matches to be considered for finding a possible alignment split.")
        ...
        ("trim-pe"                  , bpo::value<bool>(&trimPEAdapters)->default_value(trimPEAdapters),
                "Trim overhanging ends of PE alignments")
        ...
        ("disable-resume"     , bpo::value<bool>(&disableResume)->default_value(disableResume),
                "If eanbled, Isaac does not persist the state of the analysis on disk. This might save noticeable "
                "amount of runtime at the expense of not being able to use --start-from option.")
...
//                "Maximum number of mismatches allowed to accept a gapped alignment when Smith-Waterman is used.")
        ...
        ("smith-waterman-gap-size-max"   , bpo::value<unsigned>(&smithWatermanGapSizeMax)->default_value(smithWatermanGapSizeMax),
                "Maximum length of gap detectable by smith waterman algorithm.")
        ...
        ("decoy-regex" , bpo::value<std::string>(&decoyRegexString)->default_value(decoyRegexString),
                "Contigs that have matching names are marked as decoys and enjoy reduced effort. In particular: "
                "\n  - Smith waterman is not used for alignments"
                "\n  - Suspicious alignments are marked dodgy"
                "For example, to mark everything that does not begin with chr as decoy use the following regex: ^(?!chr.*)")
...
void AlignOptions::verifyMandatoryPaths(bpo::variables_map &vm)
{
    ...
    std::copy(baseCallsStringList.begin(), baseCallsStringList.end(), std::back_inserter(baseCallsDirectoryList));
    ...
    sortedReferenceXml = sortedReferenceXmlString;
...
void AlignOptions::parseSmithWatermanOptions()
{
    if ("always" == useSmithWaterman)
    {
        smartSmithWaterman = false;
        if (0 == smitWatermanGapsMax)
        {
            const format message = format("\n   *** The 'smith-waterman-gaps-max' must not be 0 for smith waterman ***\n");
            BOOST_THROW_EXCEPTION(InvalidOptionException(message.str()));
        }
    }
    ...
    {
        smartSmithWaterman = false;
        smitWatermanGapsMax = 0;
    }

    std::vector<std::string> allowedGapsMax = {"16","32","64"};
    if (allowedGapsMax.end() == std::find(allowedGapsMax.begin(), allowedGapsMax.end(), std::to_string(smithWatermanGapSizeMax)))
    {
        const format message = format("\n   *** The 'smith-waterman-gap-size-max' must not be one of: %s ***\n") % boost::join(allowedGapsMax, ",");
        BOOST_THROW_EXCEPTION(InvalidOptionException(message.str()));
    }
...
{
    referenceMetadataList.push_back(reference::ReferenceMetadata(referenceName, sortedReferenceXml, referenceMetadataList.size()));
}
...
void AlignOptions::postProcess(bpo::variables_map &vm)
{
    ...
    if(vm.count("buffer-bins"))
    {
        ISAAC_THREAD_CERR << "WARNING: --buffer-bins is not longer supported" << std::endl;
    }

    knownIndelsPath = knownIndelsPathString;
    ...
    if (!oligo::isSupportedKmerLength(seedLength))
    {
        BOOST_THROW_EXCEPTION(InvalidOptionException("\n   *** --seed-length other than " + oligo::supportedKmersString() +
            " is not supported. ***\n"));
    }
    ...
    for (std::size_t i = 0; baseCallsDirectoryList.size() > i; ++i)
    {
        flowcell::Layout fc =
            ...
            alignOptions::FastqFlowcell::createFilteredFlowcell(
                ...
                fastqQ0,
                seedLength, referenceMetadataList) :
