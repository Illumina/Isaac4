...
// Since we don't use annotations anymore, format is irrelevant. We just need to skip the xml element properly
...
void serialize(xml::XmlReader &reader, SortedReferenceMetadata::Contig &c, const unsigned int version)
{
    ...
    {
        // use KaryotypeIndex if present. In pre-Isaac-04 xml KaryotypeIndex defines the ordering in .fa
        c.index_ = reader.readElementText();
        reader += "Name";
    }
...
void serialize(xml::XmlReader &reader, SortedReferenceMetadata::Contigs &contigs, const unsigned int version)
{
    ...
    std::sort(contigs.begin(), contigs.end(),
              [](const SortedReferenceMetadata::Contig &left, const SortedReferenceMetadata::Contig &right)
...
SortedReferenceMetadata loadReferenceMetadataFromXml(
