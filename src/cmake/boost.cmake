...
find_package(Boost ${iSAAC_BOOST_VERSION_MIN} COMPONENTS ${iSAAC_BOOST_COMPONENTS})
#
# If the right version of boost is not found, it will be built from the distribution
#
if (NOT Boost_FOUND)
    if (BOOSTROOT)
        message (STATUS "BOOSTROOT is set to ${BOOSTROOT} but boost ${iSAAC_BOOST_VERSION_MIN} or higher was not found.")
        message (FATAL_ERROR "Unset BOOSTROOT or set it to the root location of boost ${iSAAC_BOOST_VERSION_MIN} or higher.")
    endif(BOOSTROOT)
    if (BOOST_ROOT)
        message (STATUS "BOOST_ROOT is set to ${BOOST_ROOT} but boost ${iSAAC_BOOST_VERSION_MIN} or higher was not found.")
        message (FATAL_ERROR "Unset BOOST_ROOT or set it to the root location of boost ${iSAAC_BOOST_VERSION_MIN} or higher.")
    endif(BOOST_ROOT)

    # Try to find it in target installation location
    resetFindBoost()
    message(STATUS "Boost ${iSAAC_BOOST_VERSION_MIN} or higher not found. Boost will be built from the distribution...")
    ...
    execute_process(COMMAND "/bin/bash"
                            ...
                            "${CMAKE_PARALLEL}"
                            "${CMAKE_CXX_COMPILER}" "${CMAKE_CXX_FLAGS} ${iSAAC_CXX_OPTIMIZATION_FLAGS}" "${INSTALL_OPTIONS}"
    ...
    set (BOOST_ROOT "${CMAKE_CURRENT_BINARY_DIR}/bootstrap" CACHE string "Redist boost" FORCE)
